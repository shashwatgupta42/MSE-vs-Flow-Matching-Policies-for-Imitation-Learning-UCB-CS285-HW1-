...
def run_training(config: TrainConfig) -> None:
    ...
    for epoch in range(config.num_epochs):
        for states, action_chunks in loader:
            ...
            if step % config.log_interval == 0:
                ...
                logger.log({"train/loss": loss.item(), "train/epoch": epoch, "train/lr": current_lr}, step=step)
                print(f"Epoch: {epoch} | Step: {step} | Loss: {loss.item():.4f} | LR: {current_lr:.6f}")

            if step % config.eval_interval == 0:
                print(f"Evaluating policy at step {step}...")
                evaluate_policy(
                    model=model,
                    normalizer=normalizer,
                    device=device,
    ...
    # Final evaluation at the end of training if not already run at the final step
    if step % config.eval_interval != 0:
        print(f"Running final evaluation at step {step}...")
        evaluate_policy(
            model=model,
            normalizer=normalizer,
            device=device,
